...
int DoAccept(void)
{
	...
	if(myPs->config.network_socket < 0) {
		printf("{SE}Code > network_socketr = GetServerSock(port);\n");
		return	-1;
	}
	i = sizeof(struct sockaddr_in);
	new_fd = accept(myPs->config.network_socket, (struct sockaddr *)&conn_addr, &i);
	if(new_fd < 0) {
		userlog(DEBUG_LOG, psName, "{SE}Accept ERROR : %s\n", strerror(errno));
		//write_NAK(new_fd);
		return	-2;
	} else {
		return	new_fd;
...
int SetClientSock(int port, char *addr)
{
	...
	//kjg_d userlog(DEBUG_LOG, psName, "socket %d, %d\n", port, clSD);
	if(clSD < 0) {
    ...
    clFD = connect(clSD, (struct sockaddr *)&serv_addr,
		sizeof(struct sockaddr_in));
	//kjg_d userlog(DEBUG_LOG, psName, "connect %d, %d\n", port, clFD);
	if(clFD < 0) {
