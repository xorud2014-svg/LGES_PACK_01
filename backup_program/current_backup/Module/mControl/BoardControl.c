...
void bRun(int group, int bd)
{
	...
	switch(myBd->phase) {
		...
		case P10:
			...
			time_cal_increment(myBd->misc.runTime_day, myBd->misc.runTime,
				(unsigned long)myPs->config.scan_period);
			myBd->misc.runTime_day = myPs->misc.tmp_inc_time_day;
			myBd->misc.runTime = myPs->misc.tmp_inc_time;
