...
int NetworkCommand_Receive(void)
{
	...
	memcpy((char *)&header, (char *)&myPs->rcvCmd.cmdBuf, sizeof(S_SIL_CLIENT_CMD_HEADER));
	cmd_size = header.body_size + sizeof(S_SIL_CLIENT_CMD_HEADER) 
		+ sizeof(S_SIL_CLIENT_CMD_TAIL);
...
int NetworkCommand_Parsing(void)
{
	...
	switch(header.cmd_id) {
		case SIL_TO_SBC_CMD_HEARTBEAT_REQ:
			rtn = rcv_cmd_heartbeat_req();
			break;
		case SIL_TO_SBC_CMD_DAQ_DATA_RPY:
			rtn = rcv_cmd_daq_data_rpy();
			//userlog(DEBUG_LOG, psName, 
			//	"SIL_TO_SBC_CMD_DAQ_DATA size[%d]\n",header.body_size);
			break;
		case SIL_TO_SBC_CMD_RESPONSE:
			rtn = rcv_cmd_response();
			//userlog(DEBUG_LOG, psName, 
			//	"SIL_TO_SBC_CMD_RESPONSE Cmd[%d]\n",header.cmd_id);
			break;
		default:
			userlog(DEBUG_LOG, psName, 
				"Can't Find Cmd[%d]\n",header.cmd_id);
...
int CmdHeaderCheck(char *rcvHeader)
{
	...
	switch(header.cmd_id) {
		case SIL_TO_SBC_CMD_HEARTBEAT_REQ:
			len = sizeof(S_SIL_CLIENT_RCV_CMD_HEARTBEAT_REQ);
			break;
		case SIL_TO_SBC_CMD_DAQ_DATA_RPY:
			len = sizeof(S_SIL_CLIENT_RCV_CMD_DAQ_DATA);
			break;
		case SIL_TO_SBC_CMD_RESPONSE:
			len = sizeof(S_SIL_CLIENT_RCV_CMD_RESPONSE);
			break;
		default:
...
void make_header(char *buf, int cmd_id, int body_size)
{
	...
	header.module_no[1] = 0x01;
	header.cmd_id = cmd_id;
	header.body_size = body_size;

	memcpy(buf, (char *)&header, sizeof(S_SIL_CLIENT_CMD_HEADER));
}
...
int	send_command(char *cmd, int cmd_size)
{
	...
	memset((char *)&header, 0x00, sizeof(S_SIL_CLIENT_CMD_HEADER));
	memcpy((char *)&header, cmd, sizeof(S_SIL_CLIENT_CMD_HEADER));
...
{
	short int AuxV_Count, AuxTH_Count, Aux_Count;
	int	cmd_size, body_size, rtn;
	S_SIL_CLIENT_SEND_CMD_MODULE_INFO	cmd;
	
	AuxV_Count = myData->mData.config.installedAuxV;
	AuxTH_Count = myData->mData.config.installedTH;
	Aux_Count = AuxV_Count + AuxTH_Count;
	
		userlog(DEBUG_LOG, psName, "send_cmd_module_info!\n", cmd.header.cmd_id);
	cmd_size = sizeof(S_SIL_CLIENT_SEND_CMD_MODULE_INFO);
	memset((char *)&cmd, 0x00, cmd_size);
	cmd.reserved[0] = Aux_Count;
	cmd.reserved[1] = AuxV_Count;
	cmd.reserved[2] = AuxTH_Count;
//	cmd.reserved[0] = 64;
//	cmd.reserved[1] = 64;
//	cmd.reserved[2] = 0;
	cmd.reserved[3] = 0;
	cmd.header.module_no[1] = 0x01;
	cmd.tail.reserved = 0;
	cmd.tail.etx = SIL_CLIENT_ETX;
	body_size = cmd_size - sizeof(S_SIL_CLIENT_CMD_HEADER) - sizeof(S_SIL_CLIENT_CMD_TAIL);
	make_header((char *)&cmd, SBC_TO_SIL_CMD_MODULE_INFO, body_size);
...
int send_cmd_daq_start(int ch)
{
	int	cmd_size, body_size, rtn;
	S_SIL_CLIENT_RCV_CMD_START_REQ	cmd;
	
	cmd_size = sizeof(S_SIL_CLIENT_RCV_CMD_START_REQ);
	memset((char *)&cmd, 0x00, cmd_size);
	cmd.tail.reserved = 0;
	cmd.tail.etx = SIL_CLIENT_ETX;
	cmd.header.module_no[1] = ch;
	body_size = 0;
	make_header((char *)&cmd, SBC_TO_SIL_CMD_START, body_size);
	rtn = send_command((char *)&cmd, cmd_size);
	userlog(DEBUG_LOG, psName, "CMD[%d] (START)\n", cmd.header.cmd_id);
	if(rtn < 0) {
		userlog(DEBUG_LOG, psName, "CMD[%d] SEND FAIL!(START)\n", cmd.header.cmd_id);
		return -1;
	}
	return 0;
}

int send_cmd_daq_stop(int ch)
{
	int	cmd_size, body_size, rtn;
	S_SIL_CLIENT_RCV_CMD_STOP_REQ	cmd;
	
	cmd_size = sizeof(S_SIL_CLIENT_RCV_CMD_STOP_REQ);
	memset((char *)&cmd, 0x00, cmd_size);
	cmd.tail.reserved = 0;
	cmd.tail.etx = SIL_CLIENT_ETX;
	cmd.header.module_no[1] = ch;
	body_size = 0;
	make_header((char *)&cmd, SBC_TO_SIL_CMD_STOP, body_size);
	rtn = send_command((char *)&cmd, cmd_size);
	if(rtn < 0) {
		userlog(DEBUG_LOG, psName, "CMD[%d] SEND FAIL!(STOP)\n", cmd.header.cmd_id);
		return -1;
	}
	return 0;
}
...
{ 
	int	cmd_size, body_size, rtn;
	S_SIL_CLIENT_SEND_CMD_RESPONSE	cmd;
	
	cmd_size = sizeof(S_SIL_CLIENT_SEND_CMD_RESPONSE);
	body_size = cmd_size - sizeof(S_SIL_CLIENT_CMD_HEADER) - sizeof(S_SIL_CLIENT_CMD_TAIL);
	memset((char *)&cmd, 0, cmd_size);

	make_header((char *)&cmd, SBC_TO_SIL_CMD_RESPONSE, body_size);
...
int	rcv_cmd_heartbeat_req(void)
{
	myPs->pingCount = 0;
	myPs->pingTimer = myData->mData.misc.timer_1sec;
	return send_cmd_response(SIL_TO_SBC_CMD_HEARTBEAT_REQ, P1_CD_ACK);
}

int rcv_cmd_response(void)
{
	S_SIL_CLIENT_RCV_CMD_RESPONSE cmd;

	memcpy((char *)&cmd, (char *)&myPs->rcvCmd.cmd,
		sizeof(S_SIL_CLIENT_RCV_CMD_RESPONSE));
...
{
	int rtn=0, i;
	S_SIL_CLIENT_RCV_CMD_DAQ_DATA cmd;

	myPs->pingCount = 0;
	myPs->pingTimer = myData->mData.misc.timer_1sec;

	memcpy((char *)&cmd, (char *)&myPs->rcvCmd.cmd,
		sizeof(S_SIL_CLIENT_RCV_CMD_DAQ_DATA));
	//shhw_231127s
	for(i= 0; i < MAX_DAQ_CHANNEL; i++) {		//DAQ_MAX: 256CH
		myData->sil_daq_val[i] = cmd.Module.daq[i];				
	}
	for(i= 0; i < MAX_TC_CHANNEL; i++) {		//TC_MAX:	100CH
		myData->sil_tc_val[i] = cmd.Module.tc[i];				
	}
	/*for(i=0;i<MAX_SENSOR_CHANNEL;i++) {
		myData->sil_daq_val[i] = cmd.Module.temp[i];				//shhw_221114
		//myData->sil_daq_val[i] = cmd.Module.temp[i] * 1000;		//mv -> uv
	}*/
	//shhw_231127e
	return rtn;
}
