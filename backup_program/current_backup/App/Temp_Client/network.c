...
int InitNetwork(void)
{
	...
	sleep(2);	
//	SetServerInfo(myPs->config.networkPort);
	myPs->config.network_socket
		= GetServerSock(myPs->config.networkPort);
    if(myPs->config.network_socket < 0) {
		close(myPs->config.network_socket);
	    userlog(DEBUG_LOG, psName,
			"Can not initialize network(network Port) : %d\n",
	...
	myPs->config.fd_socket = DoAccept();
    if(myPs->config.fd_socket < 0) {
		close(myPs->config.network_socket);
	    userlog(DEBUG_LOG, psName,
			"Can not initialize network(network Port) : %d\n",
	...
	userlog(DEBUG_LOG, psName,
		"Server socket connected socketNo[%d]\n",
		myPs->config.network_socket);
	userlog(DEBUG_LOG, psName,
		"FD socket connected socketNo[%d]\n",
		myPs->config.fd_socket);
...
/*	if(myPs->config.CmdRcvLog == P1) {
		switch(cmd_id){
			...
			sprintf(temp_log, "[T->M] CmdID=%3d Header=%s Body=%s ",
				cmd_id, (char *)&buf, (char *)&body);
			userlog(DEBUG_LOG, psName, "%s\n",(char *)&temp_log);
			break;
		}
	}
*/	
	...
	switch(cmd_id) {
		case TEMP_TO_SBC_CMD_HEARTBEAT_RPY:
			...
			break;
		case TEMP_TO_SBC_CMD_TEMP_DATA_RPY:
			rtn = rcv_cmd_temp_data_rpy();
			break;
		default:
			userlog(DEBUG_LOG, psName, 
				"Can't Find Cmd[%d]\n",cmd_id);
...
int CmdHeaderCheck(char *rcvHeader)
{
	...
	switch(cmd_id) {
		case TEMP_TO_SBC_CMD_HEARTBEAT_RPY:
			...
			break;
		case TEMP_TO_SBC_CMD_TEMP_DATA_RPY:
			len = sizeof(S_TEMP_CLIENT_RCV_CMD_TEMP_DATA);
			break;
		default:
				userlog(DEBUG_LOG, psName, 
...
void make_header(char *buf, int cmd_id, int body_size)
{
	...
	sprintf(temp, "%04d", cmd_id);
	memcpy((char *)&header.cmd_id, temp, sizeof(header.cmd_id));

	memset(temp, 0x00, sizeof(temp));
	sprintf(temp, "%04d", body_size);
	memcpy((char *)&header.body_size, temp, sizeof(header.body_size));
...
//	char	body[MAX_P1_RECV_PACKET_LENGTH];
//	char	temp_log[MAX_P1_RECV_PACKET_LENGTH];
//	char 	buf[12], tmp[12], header1[32];
//	int i, rtn, cmd_id, size;
	S_TEMP_CLIENT_CMD_HEADER header;
...
/*	if(myPs->config.CmdSendLog == P1) {
		...
		memset(buf, 0x00, sizeof(buf));
		memcpy(buf, (char *)&header.cmd_id, sizeof(header.cmd_id));
		cmd_id = atoi(buf);

		switch(cmd_id){
			case SBC_TO_TEMP_CMD_TEMP_DATA_REQ:
				break;
			default:
//				sprintf(temp_log, "CmdID=%3d Header=%s Body=%s CRC=%x%x",
//					cmd_id, (char *)&header1, (char *)&body, (unsigned char)tmp[0], (unsigned char)tmp[1]);
...
{ 
	...
	body_size = 0;
	make_header((char *)&cmd, SBC_TO_TEMP_CMD_HEARTBEAT_REQ, body_size);
	rtn = send_command((char *)&cmd, cmd_size);
	if(rtn < 0) {
		userlog(DEBUG_LOG, psName, "CMD[%d] SEND FAIL!\n", atoi(cmd.header.cmd_id));
		return -1;
	}
	return 0;
}
...
{ 
	...
	body_size = 0;
	make_header((char *)&cmd, cmd_id, body_size);
...
int send_cmd_temp_data_req(void)
{
	...
	memset((char *)&cmd, 0x00, cmd_size);
	make_header((char *)&cmd, SBC_TO_TEMP_CMD_TEMP_DATA_REQ, body_size);

//	make_crc(0, (char *)&cmd, cmd_size - 2);
	rtn = send_command((char *)&cmd, cmd_size);
	if(rtn < 0) {
		userlog(DEBUG_LOG, psName, "cmd send error!!! %d cmd_id[%d]\n", rtn,
			SBC_TO_TEMP_CMD_TEMP_DATA_REQ);
	}
	return rtn;
}
...
{
	int rtn=0, i, start_ch, Module, ch, cnt, j, sum, max, min;
	float val1;
	char tmp[12];
	S_TEMP_CLIENT_RCV_CMD_TEMP_DATA cmd;

	myPs->pingCount = 0;
	myPs->pingTimer = myData->mData.misc.timer_1sec;

	memcpy((char *)&cmd, (char *)&myPs->rcvCmd.cmd[1],
		sizeof(S_TEMP_CLIENT_RCV_CMD_TEMP_DATA));

	start_ch
		= myData->Temp_Client[0].config.installedTemp * myPs->misc.TempNo;

	cnt = myData->COM.com_port[AUX_TEMP].temp_cnt;
	myData->COM.com_port[AUX_TEMP].temp_cnt++;
	if(myData->COM.com_port[AUX_TEMP].temp_cnt >= 3){
		myData->COM.com_port[AUX_TEMP].temp_cnt = 0;
		myData->COM.com_port[AUX_TEMP].temp_cnt_flag = P1;
	}
	for(i = start_ch; i < start_ch + myPs->config.installedTemp; i++){
		if(myPs->misc.TempNo == 0){
			Module = i / 10;
			ch = i % 10;
		}else{
			Module = (i - start_ch) / 10;
			ch = (i - start_ch) % 10;
		}
		if(Module >= MAX_MODULE_PER_TEMP_CONV_BD) continue;
		memset(tmp, 0x00, sizeof(tmp));	
		memcpy((char *)&tmp, (char *)&cmd.Module[Module].temp[ch][1], 7);
		val1 = atof(tmp)*1000.0;
		if(myData->mData.config.multi_temp_cali == 1) { //shh_211221s
			if(val1 != 9999.900) {
				cFind_Temp_Cali_Data((long)val1, i);     //khj_210802
			}
		}	//shh_211221e
		myData->COM.com_port[AUX_TEMP].temp_value[i][cnt]
			= (long)(val1 * myData->measure_cali[0][i].factor + myData->measure_cali[0][i].offset);

		if(myData->COM.com_port[AUX_TEMP].temp_cnt_flag == P0){
			...
			for(j = 0; j < 3; j++){
				if(j == 0){
					max = myData->COM.com_port[AUX_TEMP].temp_value[i][j];
					min = myData->COM.com_port[AUX_TEMP].temp_value[i][j];
				}else{
					if(myData->COM.com_port[AUX_TEMP].temp_value[i][j] > max){
						max = myData->COM.com_port[AUX_TEMP].temp_value[i][j];
					}
					if(myData->COM.com_port[AUX_TEMP].temp_value[i][j] < min){
						min = myData->COM.com_port[AUX_TEMP].temp_value[i][j];
					}
				}
				sum += myData->COM.com_port[AUX_TEMP].temp_value[i][j];
			}
			myData->COM.com_port[AUX_TEMP].value[i] = (long)(sum - max -min);
		}

/*		myData->COM.com_port[AUX_TEMP].test_value[i]
			= val1 * myData->measure_cali[0][i].factor + myData->measure_cali[0][i].offset;
		if(((myData->COM.com_port[AUX_TEMP].test_value[i] - myData->COM.com_port[AUX_TEMP].value[i]) >= 5000)
			|| ((myData->COM.com_port[AUX_TEMP].test_value[i] - myData->COM.com_port[AUX_TEMP].value[i]) <= -5000)) {
			if(myData->COM.com_port[AUX_TEMP].fault_count[i] < 5) {
				myData->COM.com_port[AUX_TEMP].fault_count[i]++;
			} else {
				myData->COM.com_port[AUX_TEMP].value[i] = myData->COM.com_port[AUX_TEMP].test_value[i];
				myData->COM.com_port[AUX_TEMP].fault_count[i] = 0;
			}
		} else {
			myData->COM.com_port[AUX_TEMP].value[i] = myData->COM.com_port[AUX_TEMP].test_value[i];
			myData->COM.com_port[AUX_TEMP].fault_count[i] = 0;
