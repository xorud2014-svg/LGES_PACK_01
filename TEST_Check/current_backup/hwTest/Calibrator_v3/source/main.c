...
/*	cmd_dav[0] = 16383;
	...
	if(type == CALI_VOLTAGE) {
		...
		printf("cmd_adv_org[%d] %f %f %f\n", group,
			cmd_adv_org[group][0]/1000.0,
			cmd_adv_org[group][1]/1000.0,
			cmd_adv_org[group][2]/1000.0);
			...
			switch(systemModel) {
				...
				case SW_DEMO_5V3A:
					...
					if(val >= 0.0) {
						...
						//SetMainDA_L(19988); //6.1V
						//SetMainDA_L(24576); //7.5V
						SetMainDA_L(32700); //kjgd
					} else {
	    ...
	    case 8: //scan type select
			...
			if(calbox == CAL_BOX_SBC_CTRL) {
				...
				switch(systemModel) {
					...
					case SW_DEMO_5V3A:
						if(scanType == SCAN_VOLTAGE) {
							outb(0x00, 0x611); //CC/CV Relay Togle
						} else { //SCAN_CURRENT
							outb(0x80, 0x611); //CC/CV Relay Togle
						}
						break;
					default:
						if(scanType == SCAN_VOLTAGE) {
...
int main(int argc, char *argv[])
{
	...
	switch(systemModel) {
		...
		case SW_DEMO_5V3A:
			...
			outb(0x00, 0x604); //inverter capacitor relay off, fan off
			sleep(1);
