...
#include "rt_can.h"
#include "rt_canP.h"
//#include "../../INC/datastore.h"	//csk_200120

//extern volatile S_SYSTEM_DATA	*myData;		//csk_200120
...
int sja1000_baud_rate(int rate, int flags, int set_sjw, int manual_can_bitconfig, int nominal_brp, int nominal_tseg1, int nominal_tseg2, int nominal_sjw)
{//ksh_241112 ADD int manual_can_bitconfig, int norminal_brp, int norminal_tseg1, int norminal_tseg2, int nominal_sjw
	int best_error = 1000000000;
	int error;
	int best_tseg=0, best_brp=0, best_rate=0, brp=0;
	int tseg=0, tseg1=0, tseg2=0;
	int _manual_can_bitconfig = manual_can_bitconfig;
...
//	if (rate < ((100000 + 125000) / 2)) sjw = 1; //112500i
	
	//for initalization
	if(_manual_can_bitconfig == 1 && nominal_brp == 0 && nominal_tseg1 == 0 && nominal_tseg2 == 0 && nominal_sjw == 0) _manual_can_bitconfig = 0; //ksh_241112
	switch(manual_can_bitconfig) { //ksh_241112
		case 0:	//OFF
			...
			break;
		case 1: //Manual Can BitConfirguration ON
			sjw = nominal_sjw;
			best_brp = nominal_brp;
			tseg1 = nominal_tseg1;
			tseg2 = nominal_tseg2;
			break;
	}
...
int rt_can_set_param(int can, int baud, int sjw, int extended, int inc, int manual_can_bitconfig, int nominal_brp, int nominal_tseg1, int nominal_tseg2, int nominal_sjw)
{//ksh_241112 ADD int manual_can_bitconfig, int nominal_brp, int nominal_tseg1, int nominal_tseg2, int nominal_sjw
...
//	rt_can_irq_off(state); //kjg_120427
	...
	btr0btr1 = (unsigned short)sja1000_baud_rate(baud, 0, sjw, manual_can_bitconfig, nominal_brp, nominal_tseg1, nominal_tseg2, nominal_sjw); //ksh_241112
...
int rt_can_setup(int can, int baud, int sjw, unsigned char extended, int manual_can_bitconfig, int nominal_brp, int nominal_tseg1, int nominal_tseg2, int nominal_sjw)
{//ksh_241112 ADD int manual_can_bitconfig, int norminal_brp, int norminal_tseg1, int norminal_tseg2, int nominal_sjw
...
int init_module(void)
{
    ...
    for(i=0; i < RT_CAN_CNT; i++) {
		...
		if(p->used > 0) {
	    	...
	    	rt_can_setup(i, 0, 0, 0, 0, 0, 0, 0, 0); //ksh_24112
		}
    }
    ...
    if(0 == errorcode) {
		...
		/*if((myData->CAN.config.CmdSendLog > 0) || (myData->CAN.config.CmdRcvLog > 0)
				|| (myData->CAN.config.CmdSendLog_Hex > 0) || (myData->CAN.config.CmdRcvLog_Hex > 0)
				|| (myData->CAN.config.CommCheckLog > 0)) {		//csk_200120
			printk(KERN_INFO
				"rt_can: CAN driver (version " VERSION ") sucessfully loaded.\n");
		}*/
    } else {
...
void cleanup_module(void)
{
    ...
    for(i=0; i < RT_CAN_CNT; i++) {
		...
		if(p->used > 0) {
		    rt_can_free_irq(p->irq);
			//rt_can_setup(i, 0, 0, 0);
		    rt_can_setup(i, 0, 0, 0, 0, 0, 0, 0, 0); //ksh_241112
	    	release_region(p->port, 8);
		}
    }
