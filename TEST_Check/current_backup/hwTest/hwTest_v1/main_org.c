...
typedef union ADData_tag {
	short int	val;
	unsigned char	data[2];
} ADData;

typedef struct s_sub_sens_v_org_tag {
	unsigned char		zero_val[6];
	ADData				ref_val[3][4][4];
	ADData				sub_val[8][4][4];
...
int main(void)
{
	...
	union	tmpData adData;
    struct	timeval tv;
    fd_set	rfds;
	FILE *fp;
	S_SUB_SENS_V_ORG	daq;
	...
	while(1) {
		...
		if(strcmp(tmp, "write") == 0) {
			...
			outb(0x00,o_addr);
		} else if(strcmp(tmp, "daq1") == 0) {
			for(i= 0; i < 7; i++){
				inb(0x710);
			}
			for(i= 0; i < 88; i++){
				daq.ref_val[3][4][i] = inb(0x710);
			}
			for(i= 0; i < 4; i++){
				printf("%d ", (int)daq.ref_val[3][4][i].val);
			}
