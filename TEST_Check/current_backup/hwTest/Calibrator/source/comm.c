...
extern char rxData[256], txData[256];
...
int rxsettings(char *buf, int length)
{
    ...
    fd = opentty();
    if(fd < 0) {
		printf("Error: cannot open tty(Tray Switch)\n");
		return -1;
    }
    Send(fd, buf, length);
	tcdrain(fd);
    ...
    do {
		rtn = read(fd, rxData, sizeof rxData);
		if(rtn == -1) perror("read"); 
		if(rxData[rtn - 1] == 0x03) {
			rtn = 0;
			break;
		}
		MaxCnt++;
		if(MaxCnt >= 5) {
			rtn = -2;
			printf("Read Error : tty(TraySwitch)\n");
 /*			
			for(i=0; i <length ; i++)
				printf("rxData[%d]:%x\n",i,rxData[i]);
				printf("\n");
