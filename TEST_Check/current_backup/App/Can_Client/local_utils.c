...
int Initialize(int argc, char *argv[])
{
	...
	myPs = &(myData->Can_Client);
	
	Init_SystemMemory(CanNo);
	
	if(Read_CanClient_Config() < 0) return -3;
...
void Init_SystemMemory(int CanNo)
{
	memset((char *)&psName[0], 0, PROCESS_NAME_SIZE);
	switch(CanNo){
		case 1:
			strcpy(psName, "CAN1");
		    myPs->misc.psSignal = APP_SIG_CAN_CLIENT_PROCESS;
			break;
		default:
			printf("default %d\n", CanNo);
			break;
...
int	Read_CanClient_Config(void)
{
    int tmp;
	...
	strcat(fileName, "/CanClient_Config");
    if((fp = fopen(fileName, "r")) == NULL) {
		userlog(DEBUG_LOG, psName, "CanClient_Config file read error\n");
		return -1;
	}
