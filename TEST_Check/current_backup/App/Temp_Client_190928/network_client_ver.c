...
int rcv_cmd_temp_data_rpy(void)
{
	...
	for(i = start_ch; i < myPs->config.installedTemp; i++){
		...
		temp = val1 * myData->measure_cali[0][i].factor + myData->measure_cali[0][i].offset;
		myData->aux[AUX_TEMP].value[i] = temp;
		myData->COM.com_port[AUX_TEMP].value[i] = temp;
/*		if(((myData->aux[AUX_TEMP].test_value[i] - myData->aux[AUX_TEMP].value[i]) >= 5000)
			|| ((myData->aux[AUX_TEMP].test_value[i] - myData->aux[AUX_TEMP].value[i]) <= -5000)) {
			if(myData->aux[AUX_TEMP].fault_count[i] < 5) {
				myData->aux[AUX_TEMP].fault_count[i]++;
			} else {
				myData->aux[AUX_TEMP].value[i] = myData->aux[AUX_TEMP].test_value[i];
				myData->aux[AUX_TEMP].fault_count[i] = 0;
			}
		} else {
			myData->aux[AUX_TEMP].value[i] = myData->aux[AUX_TEMP].test_value[i];
			myData->aux[AUX_TEMP].fault_count[i] = 0;
