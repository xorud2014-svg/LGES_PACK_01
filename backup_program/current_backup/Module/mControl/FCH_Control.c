...
void FCH_Control(int main_slot)
{
#if defined __COC__
	...
	if(slot == 0) {
		cmd = FCH_CMD_RD1;
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		for(ch=0; ch < max_ch; ch++) FCH_Read(ch, 0);
		if(time_delay != 0) usleep(time_delay);

		cmd = FCH_CMD_AD_VALUE3;
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		for(ch=0; ch < max_ch; ch++) FCH_Read(ch, 0);

		for(ch=0; ch < myPs->config.installedCh; ch++) FCH_CalChAverage(ch);
	} else if(slot == 1) {
		switch(myData->fch.signal[FCH_SIG_GET_PHASE1]) {
			case P0:
				cmd = FCH_CMD_AD_VALUE1;
				myData->fch.signal[FCH_SIG_GET_PHASE1]++;
				break;
			case P1:
				cmd = FCH_CMD_AD_VALUE2;
				myData->fch.signal[FCH_SIG_GET_PHASE1]++;
				break;
			case P2:
				cmd = FCH_CMD_AD_VALUE4;
				myData->fch.signal[FCH_SIG_GET_PHASE1]++;
				break;
			case P3:
				cmd = FCH_CMD_AD_VALUE5;
				myData->fch.signal[FCH_SIG_GET_PHASE1]++;
				break;
			default:
				cmd = FCH_CMD_AH_WH;
				myData->fch.signal[FCH_SIG_GET_PHASE1] = P0;
				break;
		}
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		...
		switch(myData->fch.signal[FCH_SIG_GET_PHASE2]) {
			case P0:
				cmd = FCH_CMD_AVG_V_I;
				myData->fch.signal[FCH_SIG_GET_PHASE2]++;
				break;
			case P1:
				cmd = FCH_CMD_MAX_V_I;
				myData->fch.signal[FCH_SIG_GET_PHASE2]++;
			...
			default:
				cmd = FCH_CMD_ETC2;
				myData->fch.signal[FCH_SIG_GET_PHASE2] = P0;
				break;
		}
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		...
		cmd = FCH_CMD_TEST_COND1;
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		for(ch=0; ch < max_ch; ch++) FCH_Read(ch, 0);
	} else if(slot == 3) {
		cmd = FCH_CMD_TEST_COND2;
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		for(ch=0; ch < max_ch; ch++) FCH_Read(ch, 0);
		if(time_delay != 0) usleep(time_delay);

		cmd = FCH_CMD_TEST_COND3;
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		for(ch=0; ch < max_ch; ch++) FCH_Read(ch, 0);
	} else if(slot == 4) {
		cmd = FCH_CMD_TEST_COND4;
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		for(ch=0; ch < max_ch; ch++) FCH_Read(ch, 0);
		if(time_delay != 0) usleep(time_delay);

		cmd = FCH_CMD_TEST_COND5;
		for(ch=0; ch < max_ch; ch++) FCH_Write(ch, cmd);
		if(time_delay != 0) usleep(time_delay);
		for(ch=0; ch < max_ch; ch++) FCH_Read(ch, 0);
	}
...
void FCH_Write(int ch, int cmd)
{
	...
	if(cmd == FCH_CMD_TEST_COND4) {
		if(myCh->op.state == C_CALI) {
			if(myCh->op.phase == P10 && myCh->signal[C_SIG_CALI_PHASE] == P1) {
				cmd = FCH_CMD_DA_VALUE3;
				myData->fch.ch[ch].da_val[0] = myCh->misc.cmd_v[0];
				myData->fch.ch[ch].da_val[1] = myCh->misc.cmd_v[1];
			}
			if(myCh->op.phase == P11 && myCh->signal[C_SIG_CALI_PHASE] == P1) {
				...
				if(myData->cali[ch].mode == CALI_MODE_ONLY_CHECK) {
					tmp = (double)myCh->misc.cmd_v[range]
						* myData->cali[ch].data[0][range].DA_A[i]
						+ myData->cali[ch].data[0][range].DA_B[i];
				} else {
					tmp = (double)myCh->misc.cmd_v[range]
				...
				if(range == (RANGE1-1)) {
					myData->fch.ch[ch].da_val[0] = (long)tmp;
					myData->fch.ch[ch].da_val[1] = myCh->misc.cmd_v[1];
				} else {
					myData->fch.ch[ch].da_val[0] = myCh->misc.cmd_v[0];
					myData->fch.ch[ch].da_val[1] = (long)tmp;
				}
			}
	...
	if(cmd == FCH_CMD_TEST_COND5) {
		if(myCh->op.state == C_CALI) {
			if(myCh->op.phase == P10 && myCh->signal[C_SIG_CALI_PHASE] == P1) {
				cmd = FCH_CMD_DA_VALUE4;
				myData->fch.ch[ch].da_val[2] = myCh->misc.cmd_i[0];
				myData->fch.ch[ch].da_val[3] = 0;
			}
			if(myCh->op.phase == P11 && myCh->signal[C_SIG_CALI_PHASE] == P1) {
				cmd = FCH_CMD_DA_VALUE4;
				myData->fch.ch[ch].da_val[2] = myCh->misc.cmd_i[0];
			...
			if(myCh->op.phase == P21 && myCh->signal[C_SIG_CALI_PHASE] == P1) {
				...
				if(myData->cali[ch].mode == CALI_MODE_ONLY_CHECK) {
					tmp = (double)myCh->misc.cmd_i[0]
						* myData->cali[ch].data[1][range].DA_A[i]
						+ myData->cali[ch].data[1][range].DA_B[i];
				} else {
					tmp = (double)myCh->misc.cmd_i[0]
						* myData->cali[ch].tmpData[1][range].DA_A[i]
						+ myData->cali[ch].tmpData[1][range].DA_B[i];
				}
				myData->fch.ch[ch].da_val[2] = (long)tmp;
				myData->fch.ch[ch].da_val[3] = 0;
			}
		} else if(myCh->op.state == C_STANDBY || myCh->op.state == C_PAUSE) {
			if(myCh->signal[C_SIG_CALI_UPDATE] != P0) {
	...
	switch(cmd) {
		...
		case FCH_CMD_TEST_COND5:
			...
			break;
		case FCH_CMD_CALI_UPDATE:
			wr_data[2] = myData->fch.ch[ch].cali_type;
			wr_data[3] = myData->fch.ch[ch].cali_range;
			wr_data[4] = myData->fch.ch[ch].cali_mode;
			wr_data[5] = myData->fch.ch[ch].cali_index;
			memcpy((char *)&wr_data[6],
				(char *)&myData->fch.ch[ch].cali_data, 4);
			break;
		default:
			break;
	}
...
void FCH_Read(int ch, int cmd)
{
	...
	switch(cmd) {
		case FCH_CMD_SYSTEM_MODEL_REPLY:
			break;
		case FCH_CMD_FIRMWARE_VERSION_REPLY:
			memcpy((char *)&myData->fch.ch[ch].firmware_version[0],
				(char *)&rd_data[2], 8);
			break;
		case FCH_CMD_RD1_REPLY:
			myData->fch.ch[ch].i_val[0] = rd_data[2];
			myData->fch.ch[ch].i_val[1] = rd_data[3];
			break;
		case FCH_CMD_AD_VALUE1_REPLY:
			memcpy((char *)&myData->fch.ch[ch].ad_val[0],
				(char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].ad_val[1],
				(char *)&rd_data[6], 4);
			break;
		case FCH_CMD_AD_VALUE2_REPLY:
			memcpy((char *)&myData->fch.ch[ch].ad_val[2],
				(char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].ad_val[3],
				(char *)&rd_data[6], 4);
			break;
		case FCH_CMD_AD_VALUE3_REPLY:
			memcpy((char *)&myData->fch.ch[ch].ad_val[4],
				(char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].ad_val[5],
				(char *)&rd_data[6], 4);

			tmpV = myData->fch.ch[ch].ad_val[4];
			...
			myCh->op.Isens = myData->fch.ch[ch].ad_val[5];
			myCh->misc.tmpIsens = myData->fch.ch[ch].ad_val[5];
			if(myCh->op.state == C_CALI) {
				myData->cali[ch].orgAD[0] = tempV;
				myData->cali[ch].orgAD[1] = myCh->op.Isens;
			}
			break;
		case FCH_CMD_AD_VALUE4_REPLY:
			memcpy((char *)&myData->fch.ch[ch].ad_val[6],
				(char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].ad_val[7],
				(char *)&rd_data[6], 4);
			break;
		case FCH_CMD_AD_VALUE5_REPLY:
			memcpy((char *)&myData->fch.ch[ch].ad_val[8],
				(char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].ad_val[9],
				(char *)&rd_data[6], 4);
			break;
		case FCH_CMD_AH_WH_REPLY:
			memcpy((char *)&myData->fch.ch[ch].ah, (char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].wh, (char *)&rd_data[6], 4);
			break;
		case FCH_CMD_AVG_V_I_REPLY:
			memcpy((char *)&myData->fch.ch[ch].avg_v, (char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].avg_i, (char *)&rd_data[6], 4);
			break;
		case FCH_CMD_MAX_V_I_REPLY:
			memcpy((char *)&myData->fch.ch[ch].max_v, (char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].max_i, (char *)&rd_data[6], 4);
			break;
		case FCH_CMD_MIN_V_I_REPLY:
			memcpy((char *)&myData->fch.ch[ch].min_v, (char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].min_i, (char *)&rd_data[6], 4);
			break;
		case FCH_CMD_ETC1_REPLY:
			break;
		case FCH_CMD_TEST_COND3_REPLY:
			memcpy((char *)&myData->fch.ch[ch].v_power_bus,
				(char *)&rd_data[2], 4);
			memcpy((char *)&myData->fch.ch[ch].v_output_bus,
				(char *)&rd_data[6], 4);
			break;
		case FCH_CMD_TEST_COND5_REPLY:
			...
			} else if(myCh->signal[C_SIG_OUT_SWITCH] == P67) { //lki_111111
				if(rd_data[2] == P68) myCh->signal[C_SIG_OUT_SWITCH] = P68;
			...
			//kjg_120105
			myCh->signal[C_SIG_RECV_OUT_SWITCH_1] = rd_data[2];
			myCh->signal[C_SIG_RECV_OUT_SWITCH_2] = rd_data[3];
			myCh->signal[C_SIG_OUT_SWITCH_PHASE] = rd_data[4];
			break;
		default:
			break;
	}
	...
	if(myData->AppControl.config.debugType == 1) {
		if(myCh->signal[C_SIG_OUT_SWITCH] == P1) {
			...
			myCh->signal[C_SIG_OUT_SWITCH] = P68;
		} else if(myCh->signal[C_SIG_OUT_SWITCH] == P71) {
...
void FCH_send_cali_data(int ch)
{
#if defined __COC__
	...
	switch(mode) {
		case P1:
			...
			range = 0;
			if(phase == P0) {
				myData->fch.ch[ch].cali_type = type;
				myData->fch.ch[ch].cali_range = range;
				myData->fch.ch[ch].cali_mode = mode; //setPointNum
				myData->fch.ch[ch].cali_index = phase;
				myData->fch.ch[ch].cali_data = (long)myData->cali[ch]
					.data[type][range].point.setPointNum;
			} else {
				myData->fch.ch[ch].cali_type = type;
				myData->fch.ch[ch].cali_range = range;
				myData->fch.ch[ch].cali_mode = mode; //set_ad
				myData->fch.ch[ch].cali_index = phase;
				myData->fch.ch[ch].cali_data = (long)myData->cali[ch]
		...
		case P2:
			...
			myData->fch.ch[ch].cali_type = type;
			myData->fch.ch[ch].cali_range = range;
			myData->fch.ch[ch].cali_mode = mode; //AD_A
			myData->fch.ch[ch].cali_index = phase;
			value = (float)myData->cali[ch].data[type][range].AD_A[phase];
			memcpy((char *)&myData->fch.ch[ch].cali_data, (char *)&value, 4);
			phase++;
			if(phase == P14) {
				phase = P0;
				mode++;
		...
		case P3:
			...
			myData->fch.ch[ch].cali_type = type;
			myData->fch.ch[ch].cali_range = range;
			myData->fch.ch[ch].cali_mode = mode; //AD_B
			myData->fch.ch[ch].cali_index = phase;
		...
		case P4:
			...
			myData->fch.ch[ch].cali_type = type;
			myData->fch.ch[ch].cali_range = range;
			myData->fch.ch[ch].cali_mode = mode; //setPoint
			myData->fch.ch[ch].cali_index = phase;
			myData->fch.ch[ch].cali_data = (long)myData->cali[ch]
		...
		case P5:
			...
			myData->fch.ch[ch].cali_type = type;
			myData->fch.ch[ch].cali_range = range;
			myData->fch.ch[ch].cali_mode = mode; //DA_A
			myData->fch.ch[ch].cali_index = phase;
		...
		case P6:
			...
			myData->fch.ch[ch].cali_type = type;
			myData->fch.ch[ch].cali_range = range;
			myData->fch.ch[ch].cali_mode = mode; //DA_B
			myData->fch.ch[ch].cali_index = phase;
...
//	myCh->op.charge_AmpareHour = myData->fch.ch[ch].ah;
	...
	//uA -> watt 1mWh/div
	//nA -> watt 1uWh/div
//	myCh->op.charge_WattHour = myData->fch.ch[ch].wh;
