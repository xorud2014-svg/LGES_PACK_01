...
void msgParsing_Module_to_DataSave(int fromPs, int idx, S_MSG_VAL *RecvMsg)
{
	...
	int group, ch, i, j, k, toPs, rtn=0, code, stepNo;
	int idx2=0, idxStepNo=0; //jhkw_191210
//#ifdef __B_TYPE__
...
//#endif
		...
		case MSG_MODULE_DATASAVE_CALI_CHECK_RESULT_SAVE:
			ch = RecvMsg->val[0];
//#ifdef __B_TYPE__
			count = RecvMsg->val[1]; //jhkw_200317
//#endif
			//kjg_w CaliCheckDataSave(bd, ch);
			...
			if(myData->AppControl.misc.Load_Process[PROCESS_GROUP_COA][0]
				...
				SendMsg.val[0] = ch;
//#ifdef __B_TYPE__
				SendMsg.val[1] = count; //jhkw_200317
//#endif
				send_msg(toPs, (char *)&SendMsg);
			}
		...
		case MSG_MODULE_DATASAVE_READ_PATTERN_FILE:
			...
			i = Read_Pattern_File_1(psName, ch, RecvMsg->val[1]);
#else //COA_VER_100D~
			i = read_test_cond_pattern_file_coa(psName, ch, RecvMsg->val[1], 1);
#endif
...
//20181219 KHK--------------------------------------------			
			...
			if(myData->testCond[ch].local_object[stepNo][IDX_LOC_OBJ_SOC_TRACKING_FLAG] == P1){
				i = Read_SOC_Tracking_File(psName, ch, stepNo);
			}*/
...
//----------------------------------------------------------			
			...
			break;
//20181219 KHK--------------------------------------------			
		case MSG_MODULE_DATASAVE_READ_SOC_TRACKING_FILE:
			ch = RecvMsg->val[0];
			//jhkw_201102s
			//i = Read_SOC_Tracking_File(psName, ch, RecvMsg->val[1]);
			i = Read_SOC_Tracking_File(psName, ch, RecvMsg->val[1], 0);
			//jhkw_201102e
			//jhkw_191210s
			if(i >= 0) {
				...
				myData->testCond[ch].local_object[idxStepNo][idx2] 
					...
					= myData->testCond[ch].SOC_tracking[0].maxI; //jhkw_201102
			}
			//jhkw_191210e
			memset((char *)&SendMsg, 0, sizeof(S_MSG_VAL));
			toPs = DATASAVE_TO_MODULE;
			SendMsg.msg = MSG_DATASAVE_MODULE_READ_END_SOC_TRACKING_FILE; 
			SendMsg.val[0] = ch;
			if(i >= 0) SendMsg.val[1] = P1;
			else SendMsg.val[1] = P2;
			send_msg(toPs, (char *)&SendMsg);

			userlog(DEBUG_LOG, psName, "read end  SOC Tracking file%d %d %d\n",
				SendMsg.val[0], SendMsg.val[1],
				(int)myData->cData[ch].signal[C_SIG_OUT_SWITCH]);
			break;
//----------------------------------------------------------------			
		...
		case MSG_MODULE_DATASAVE_CALI_VALUE:
			userlog(DEBUG_LOG, psName, "Cali meter_real: %ld, meter:%ld, ad:%ld\n",
				RecvMsg->val[2], RecvMsg->val[1], RecvMsg->val[0]);
			break;
