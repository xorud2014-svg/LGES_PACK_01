...
//	unsigned char		can_check[MAX_LOC_CODE];		//khj_201211
	...
	short int			dcr_delay_count; //kjg_111006_e
	...
	unsigned char		out_mux_fault; //kjhw_151021e
	unsigned char		can_fault_check_phase; //kjg_190323

	unsigned char		bms_link_flag; //kjh_160623

	//unsigned char		reserved6[3]; //jhkw_170917s
	//unsigned char		reserved6;
	unsigned char		wait_type;	//ktg_210511	// 0: No logging	1: logging
	short int			out_delay; //jhkw_170917e

	short int			delay_temp; //jhkw_181119s
	short int			delay_aux;			//aux_upper
	short int			delay_aux_lower;	//csk_190808
	short int			delay_temp_can;
	short int			delay_th; //jhkw_181119e
	...
	unsigned char		delay_count1; //kjhw_190627s
	unsigned char		delay_count2;
	//unsigned char		reserved7[2]; //kjhw_190627e
	//unsigned char		reserved7; //kjhw_190627e
	unsigned char		drop_flag; //ktg_210426
	unsigned char		cap_bank; //kjh_191014

	unsigned long		check_fault_delta_AuxV_Time_day;	//csk_190808
	unsigned long		check_fault_delta_AuxV_Time;		//csk_190808

	short int			comp_count_auxTemp1;
	short int			comp_count_auxV1;
	short int			comp_count_auxTH1;
	
	//unsigned char		abnormal_detection_count;	//ktg_210105s
	//unsigned char		reserved8[2];				//ktg_210315s
	unsigned char		cv_maxI_flag;
	unsigned char		cv_minI_flag;				//ktg_210315e

	long				avgAuxV[3]; 				//ktg_210105e	
	long				cv_maxI; 					//ktg_210315
	long				cv_minI; 					//ktg_210315
	long				saveTime_count;				//ktg_210408	//pause_save_count
	...
	unsigned char		can_drop_flag; //ktg_210706e

	unsigned char		gcac_link_flag;	//ktg_210511
	unsigned char		svdf_drop_flag;	//ktg_220214
	unsigned char		v_rising_flag;	//shhw_220916
	//unsigned char		reserved9;	//ktg_220214	//shhw_220916

	long				maxGroupAuxV[MAX_AUX_GROUP]; 		//ktg_220220s
	long				minGroupAuxV[MAX_AUX_GROUP];
	long				diffGroupAuxV[MAX_AUX_GROUP];
	short int			maxGroupAuxV_auxChNo[MAX_AUX_GROUP];
	short int			minGroupAuxV_auxChNo[MAX_AUX_GROUP]; //ktg_220220e
	long 				preV1;							//shhw_220916s
	unsigned long 		time_init1;									
	unsigned long 		time_check1;					//shhw_220916e		
	
	long 				preV2;							//shhw_220919s
	unsigned long 		time_init2;									
	unsigned long 		time_init_ori;									
	unsigned long 		time_check2;					//shhw_220919e		
} S_CH_MISC;

typedef struct s_ch_misc2_tag {
	//jhkw_180419s
	...
	//unsigned char		reserved1;	//ktg_211104
	unsigned char		freeze_flag;
	...
	unsigned char		iec_send_flag[2];
	unsigned char		cycle_pulse_count_flag;
	int					iec_data_count[2];
	long				iec_runTime[200];
	long				iec_Vsens[200];
	long				iec_Isens[200];
	//jhkw_180419e
	long				leakageI;	//ktg_211104

	long				sum_CanAmpareHour;	//ktg_220829s
	long				sum_CanWattHour;
	long				actualCanAmpareHour[MAX_TEST_STEP];
	long				actualCanWattHour[MAX_TEST_STEP];
	unsigned char		update_CanAmpareHour_flag[MAX_TEST_STEP];
	unsigned char		update_CanWattHour_flag[MAX_TEST_STEP];	//ktg_220829e
...
typedef struct s_sub_code_tag { //jhkw_220103s
	short int 			end_div_no;
	short int 			func_div;
	short int 			ch_index;
	short int 			reserved1;
	long				reserved2[2];
} S_SUB_CODE; //jhkw_220103e
...
typedef struct s_ch_operation_data_tag {
	...
	unsigned char		reserved1[3];	//shhw_220707
...
typedef struct s_auto_cali_reference_tag { //kjg_110813_s
	...
	short				ref_sens_count1;
	short				ref_sens_count2;
...
typedef struct s_board_source_tag {
	...
	long				sourceV2; //kjg_logic_type_140325_s
	long				calSourceV2;
	long				sourceI2;
	long				calSourceI2; //kjg_logic_type_140325_e
} S_BD_SOURCE;
...
typedef struct s_board_source2_tag {
	...
	long				sumV2[MAX_SOURCE_SENS_COUNT]; //kjg_logic_type_140325_s
	double				totalV2;
	long				sourceV2;
	long				sumI2[MAX_SOURCE_SENS_COUNT];
	double				totalI2;
	long				sourceI2; //kjg_logic_type_140325_e
} S_BD_SOURCE2;
...
typedef struct s_board_misc_tag {
	...
	double				Vsource2_AD_a; //kjg_logic_type_140325_s
	...
	double				Isource2_AD_b_N; //kjg_logic_type_140325_e
...
typedef struct s_ch_code_tag {
	...
	//unsigned int		compCount; //kjhw_181115
	//unsigned long		compCount; //kjhw_181115
...
typedef struct s_test_condition_tag {
	...
	unsigned char		reserved1[2]; //csk_190114d
} S_TEST_CONDITION;
...
typedef struct s_module_misc_tag {
	...
	//short int			reserved1;
	unsigned short int	slot_tic_timer;		//KHK 20191121

	int					processPointer;
	unsigned long		increment_period;
	long				timer_1sec;
	long				timer_1000ms;
	int					source_sensCount;
	...
	unsigned char		fan_active_flag_run; //jhkw_180304s
	long				fan_run_count;
	long				fan_stop_count; //jhkw_180304e
	...
	long long			timer_1tick_count;	//kjg_220706
	long long			hrt[3]; 			//kjg_220712
} S_MODULE_MISC;
...
typedef struct s_module_config_tag {
	...
	short int			dcr_count1;
	short int			dcr_t2;
	short int			dcr_count2;
	...
	short int			installedHumidity;	//khj_191205
	...
	unsigned char		division_CAN;		//kjh_140620
	unsigned char		Compare_flag;	//jhkw_201117
	unsigned char		reserved;	//jhkw_201117
	unsigned char		use_leakage_curr; //kjhw_140620
	
	long				out_precharging_V;	//kjh_150120
	long				out_capacitance;	//kjh_211021
	float 				out_prechg_ratio;	//kjh_211021
	long				reserved1;			//kjh_211021

	long				leakage_curr_I;		//ktg_211104

	unsigned char		multi_temp_cali;	//shh_211221
	unsigned char		BreakDownCheck;		//ktg_240930
	unsigned char		reserved2[2];		//ktg_240930
	//unsigned char		reserved2[3];		//shh_211221

	unsigned char		ratioP;	//0:no, 1:mW, 2:W	//kjh_211021
	short int			out_relay_delay;	//msec	//kjh_211021
	unsigned char		reserved3;	//kjh_211021
	
	double				da_max;	//kjh_211021
	double				main_amp;	//1.494	//kjh_211021
	double				v_feed[MAX_RANGE];	//kjh_211021
	double				i_feed[MAX_RANGE];	//kjh_211021
	double				i_ref[MAX_RANGE];	//kjh_211021
	
	unsigned char		caliV_rangeI;	//kjh_211021
	short int			caliV_cmdI;	//kjh_211021
	unsigned char		reserved4;	//kjh_211021
	

/*	//kjh_211021 double				daRatioV_P[MAX_RANGE]; //kjg_110814_s
...
typedef struct s_module_data_tag {
	...
	unsigned char		Auto_Update_rdy;	//ktg_231031
	unsigned char		Auto_Update_module_rdy;	//ktg_231031
	...
	long				ratioV;	//kjh_220321
	long				ratioI;	//kjh_220321
	double				ratioP;	//kjh_220321
	double				cp_to_cc;	//kjh_220321
	float				patt_ratioV;	//kjh_220321
	float				patt_ratioI;	//kjh_220321
	float				patt_ratioP;	//kjh_220321	
} S_MODULE_DATA;
...
typedef struct s_aux_set_data_tag {
	...
	long				end_upper;
	long				end_lower;
	long				vent_upper;		//csk_200113
	long				vent_lower;		//csk_200113
...
typedef struct s_th_table_tag {
	...
	unsigned char		type; //0Non, 1:NTC, 2:PTC //kjh_210317
	unsigned char		reserved[3]; //kjh_210317
...
typedef struct s_humidity_table_tag{		//khj_191205s
	unsigned char 		humidity_type;
	unsigned char 		bias_type;
	short int 			humidity_data_max_index;

	float				H_V[MAX_HUMIDITY_DATA][2];		//humidity
//	float				V_Humidity[MAX_HUMIDITY_DATA]; 	//mV
}S_HUMIDITY_TABLE;							//khj_191205e

typedef struct s_can_th_table_tag{		//ktg_220614s
	unsigned char 		can_th_type;
	unsigned char 		bias_type;
	short int 			can_th_data_max_index;

	float				CT_V[MAX_CAN_TH_DATA][2];		//can_temp
	unsigned char 		type;	//0 : None, 1: NTC, 2: PTC
	unsigned char 		reserved1[3];
}S_CAN_TH_TABLE;							//ktg_220614e

typedef struct s_pase_code_flag{
	unsigned char		flag_com[MAX_COM_CODE];	//ktg_230218s
	unsigned char		flag_loc[MAX_LOC_CODE];
	unsigned char		flag_aux[MAX_AUX_CODE];
	unsigned char		flag_can[MAX_CAN_CODE];
	unsigned char		flag_aux_sub[MAX_AUX_CODE][MAX_AUX_DATA];
	unsigned char		flag_can_sub[MAX_CAN_CODE][MAX_CAN_DATA];	//ktg_230218e
}S_PAUSE_CODE_FLAG;
#endif
