			...
			switch(functionModel) {
				...
				case 3: //I-7018Z
					...
					if(myData->AppControl.config.debugType == 220) {
						for(i=0; i < myPs->config.installedTemp; i++) {
							if(myData->test_val_l[9] >= 100000) {
								myCom->value[i] = (i+1) * 1000 
						    	+ (myData->test_val_l[9] /10);
							} else {
								myCom->value[i] = (i+1) * 1000 
						    	+ myData->test_val_l[9];
							}
					...
					} else if(myData->AppControl.config.debugType >= 230) { //shhw_230104
					//} else if(myData->AppControl.config.debugType == 230) {
						for(i= 0; i < myPs->config.installedTemp; i++) {
							myCom->value[i] = myData->sil_tc_val[i];
						}
					} else {
				#endif				
					}	
				...
				if(myData->COM.config.readType[comPort] == READ_V_V) {
					...
					switch(myData->AppControl.config.systemModel) {
						...
						case F_SDI_5V_450A_200A_100A_10A:
							if(myCom->signal[COM_SIG_CALI_CURRENT_RANGE]
								>= 3) {
								send_cmd_initialize_AGILENT_34401A(comPort, 3);
							} else {
						...
						case C_SBL_5V_450A_240A_60A_10A_2:
							if(myData->COM.config.functionModel[comPort] == 0) {
								...
								if(myCom->signal[COM_SIG_CALI_CURRENT_RANGE]
									>= 1) {
									send_cmd_initialize_AGILENT_34401A(
										comPort, 3);
								} else {
									send_cmd_initialize_AGILENT_34401A(
										comPort, 5);
	...
	switch(type % 10) {
		case 0: //LED
			...
			//	&& addr == 1 && type >= 10) {
				...
				if(myData->ChAttribute[2].opType != OP_INDEPENDENT
					//|| myData->ChAttribute[3].chNo_master == 0) {
					&& myData->ChAttribute[3].chNo_master == 0) {	//ktg_200402
					val |= 0x40;
				}
		...
		case 1: //V
			...
			//if((myData->COM.config.functionModel[comPort] == 2 && type < 10
			//	&& addr == 1)) {
			//	if(myData->ChAttribute[2].chNo_master == 0) {
			//		val = 0.0;
			//	}
			//}//kjhw_170630e
			break;
		...
		case 3: //P
			...
			if(myData->mData.config.ratioP == BASE_UNIT) val = (long)(val / 100); //shh_231230
			break;
		default:
			val = 0;
			val2 = -1;
...
{
	char answer[100];
	int i, index=0, temp_bd=0, temp_ch=-1, is_plus=1, group;
...
/*
	...
	for(i=1; i < 100; i++) {
		if(answer[i] == '+') {
			...
			switch(index) {
				case 0:
					temp[temp_ch] += (answer[i] - 0x30) * 10000;
					break;
				case 1:
					temp[temp_ch] += (answer[i] - 0x30) * 1000;
					break;
				case 2:
					temp[temp_ch] += (answer[i] - 0x30) * 100;
					break;
				case 3:
					temp[temp_ch] += (answer[i] - 0x30) * 10;
					break;
				case 4:
					temp[temp_ch] += (answer[i] - 0x30);
					temp[temp_ch] *= is_plus * 100; 
					break;
			}
			index++;
		}
	...
	for(temp_ch=0; temp_ch < module_per_ch; temp_ch++) {
		...
		switch(myData->AppControl.config.systemModel) {
			...
			case F_SDI_5V_400A_200A_100A_10A:
				myData->cData[i].op.temp = (long)val1;
				break;
			case F_SDI_5V_450A_200A_100A_10A:
				if(i >= 0 && i <= 11) {
					myData->cData[i].op.temp = (long)val1;
					...
					myData->cData[i-4].op.temp = (long)val1;
				} else {
				}
				myCom->value[i] = (long)val1; //kjg_d
				break;
			case F_SDI_5V_400A_200A_100A_10A_2:
				...
				if(group == 0) {
					if(i == 0) {
						...
						myData->cData[i].op.temp = (long)val2;
					} else {
						myData->cData[i].op.temp = (long)val1;
					}
				} else {
					if(i == 0) {
						myData->cData[1].op.temp = (long)val1;
					} else if(i == 1) {
						myData->cData[0].op.temp = (long)val1;
						val2 = val1;
					} else if(i == 2) {
						myData->cData[i].op.temp = (long)val2;
					} else {
...
void COM_Signal_Check_PLC_Q03UD(int comPort)
{
	switch(myCom->signal[COM_SIG_PLC_REQUEST_PHASE]) {
		case PLC_IDLE:
			myCom->misc.plc_idle_time += myPs->config.scan_period;
			if(myCom->misc.plc_idle_time >= 500){
				myCom->misc.plc_idle_time = 0;
				myCom->signal[COM_SIG_PLC_REQUEST_PHASE] = PLC_FAULT_READ_SEND;
			}
			break;
		case PLC_DOOR_OPEN_SEND:
			send_cmd_request_PLC_Q03UD(JW, PLC_DOOR_OPEN, 1, comPort);
			myCom->signal[COM_SIG_PLC_REQUEST_PHASE] = PLC_DOOR_OPEN_SEND_CHECK;
			break;
		case PLC_DOOR_CLOSE_SEND:
			send_cmd_request_PLC_Q03UD(JW, PLC_DOOR_CLOSE, 1, comPort);
			myCom->signal[COM_SIG_PLC_REQUEST_PHASE] = PLC_DOOR_CLOSE_SEND_CHECK;
			break;
		case PLC_FAULT_READ_SEND:
			send_cmd_request_PLC_Q03UD(QR, PLC_FAULT_READ, 1, comPort);
			myCom->signal[COM_SIG_PLC_REQUEST_PHASE] = PLC_FAULT_READ_SEND_CHECK;
			break;
		case PLC_INIT_SEND:
			send_cmd_request_PLC_Q03UD(JW, PLC_INIT, 1, comPort);
			myCom->signal[COM_SIG_PLC_REQUEST_PHASE] = PLC_INIT_SEND_CHECK;
			break;
	}
}
*/
