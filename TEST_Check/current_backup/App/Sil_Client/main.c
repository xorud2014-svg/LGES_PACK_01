...
int main(int argc, char *argv[])
{
	...
	if(Initialize(argc, argv) < 0) return 0;
   	...
   	while(myData->AppControl.signal[myPs->misc.psSignal] == P1) {
		myPs->signal[SIL_SIG_NET_CONNECTED] = P0;
		myPs->signal[SIL_SIG_CLIENT_CONNECT] = P0;
...
int Send_Cmd(void)
{
	...
	if(myPs->signal[SIL_SIG_NET_CONNECTED] == P1) {
