...
#define MAX_CH_PER_BD	SKE_5V15A
#define MUX				SKE_5V15A
...
//AD5318
void SetAuxDA2(int num, short int val)
{
    int j = 0;
    unsigned char auxDAChannel, low, sync;
    if(iopl(3)) exit(1);
...
//				SetAuxDA1(-1, 0x7f);
		    ...
		    case 8: //cc(1),cv(0) mode select
				...
				if(calbox==SBC_CTRL){
					...
					if(val == 0){
						outb(0x00,0x604); //CV Relay Togle
					}else{
						outb(0x08,0x604); //CC Relay Togle
					}
				}
		    ...
		    case 12:
				...
				break;
		    case 13:
//				SetAuxDA1(chNum, (unsigned char)val);
				SetAuxDA2(chNum, (short int)val);
