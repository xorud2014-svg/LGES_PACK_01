			...
			for(group=0; group < myData->AppControl.config.totalGroup;
				...
				switch(myData->AppControl.config.systemModel) {
					...
					case F_SDI_5V_400A_200A_100A_10A_2:
						if(myData->gData[group].state == G_RUN) {
							myData->gData[group].phase = P24;
						}
						break;
					case F_SDI_5V_450A_200A_100A_10A:
						if(myPs->code == M_CD_FAULT_FAN
							|| myPs->code == M_CD_FAULT_LOAD_LINE) break;
...
void mSignalCheck_DC_FAN(void)
{
	...
	stop_count = 60000;	//10min
	run_count = 36000;	//6min
	fan_active_flag = 0;
	...
	if(fan_active_flag >= 1) {
		...
		Select_OutPoint(0, 2, O_OUT_DC_FAN, ON); //phb_230503
		myPs->misc.fan_delay_time2++;		//phb_221223 //shh_230607
	} else {
		myPs->misc.fan_delay_time2 = 0;		//phb_221223 //shh_230607
		if(myPs->misc.fan_run_count > run_count) {
			myPs->misc.fan_run_count = 0;
			myPs->misc.fan_active_flag_run = 1;
			Select_OutPoint(0, 1, O_OUT_DC_FAN, OFF);
			Select_OutPoint(0, 2, O_OUT_DC_FAN, OFF); //phb_230503
		} else {
			if(myPs->misc.fan_stop_count > stop_count) {
				myPs->misc.fan_active_flag_run = 0;
				myPs->misc.fan_stop_count = 0;
				Select_OutPoint(0, 1, O_OUT_DC_FAN, ON);
				Select_OutPoint(0, 2, O_OUT_DC_FAN, ON); //phb_230503
	...
	for(ch=0; ch < myPs->config.installedCh; ch++) {
		if(myData->cData[ch].op.state == C_RUN
			|| myData->cData[ch].op.state == C_CALI) {
			Select_OutPoint(0, 1, O_OUT_DC_FAN, ON); //jhkw_130925
			myData->delayCount_2 = 0;
		} else {
			myData->delayCount = 60000; //10min
			if(myData->delayCount_2 > myData->delayCount) {
				myData->delayCount_2 = 0;
				Select_OutPoint(0, 1, O_OUT_DC_FAN, OFF); //jhkw_130925
			}
		}
	myData->delayCount_2++;
	}*/		
...
void mSignalCheck_TOWER_LAMP(void)
{
	...
	switch(myData->AppControl.config.systemModel) {
		case F_SDI_5V_400A_200A_100A_10A_2:
			mSignalCheck_TOWER_LAMP2();
			break;
		case F_SDI_5V_450A_200A_100A_10A:
			mSignalCheck_TOWER_LAMP3();
			break;
...
void mSignalCheck_TOWER_LAMP1(void)
{
	...
	for(ch=0; ch < myPs->config.chInGroup[0]; ch++) {
		if(myData->cData[ch].op.state == C_RUN) flag_green++;
...
void mSignalCheck_PACK_ISOLATION_PHASE(void)
{
	...
	switch(myPs->signal[M_SIG_PACK_ISOLATION_PHASE]) {
		case P1:
			...
			if(myPs->signal[M_SIG_PACK_ISOLATION_COUNT] > P50) { //500ms
				myPs->signal[M_SIG_PACK_ISOLATION_COUNT] = P0;
				myPs->signal[M_SIG_PACK_ISOLATION_PHASE] = P0;
				break;
			}
			if(Read_InPoint(group, 1, I_OUT_PACK_ISOLATION) == OFF) {
		...
		case P11:
			...
			if(Read_InPoint(group, 1, I_OUT_PACK_ISOLATION) == ON) {
				myPs->signal[M_SIG_PACK_ISOLATION_PHASE]++;
			}
			break;
		case P12:
...
#ifdef __COA__
	int group;
	int flag1, flag2, flag3, flag4, flag5, iso_state;	//jhkw_160602
	S_MSG_VAL SendMsg;
	...
	switch(myPs->signal[M_SIG_DAQ_ISOLATION_REPLY_DELAY2]) {
		...
		case P200:
			...
			if(myPs->signal[M_SIG_DAQ_ISOLATION_DIV_CH] == P10) {
				...
				flag1 = Read_InPoint(group, 1, I_OUT_PACK_ISOLATION2);
				flag2 = Read_InPoint(group, 1, I_OUT_PACK_ISOLATION3);
				flag3 = Read_InPoint(group, 1, I_OUT_PACK_ISOLATION4);
				flag4 = Read_InPoint(group, 1, I_OUT_PACK_ISOLATION5);
				flag5 = Read_InPoint(group, 1, I_OUT_PACK_ISOLATION6);
			...
			SendMsg.val[1] = (int)myData->SubSensV
				.signal[SUB_SENS_SIG_DAQ_ISOLATION];
			//SendMsg.val[2] = myPs->signal[M_SIG_DAQ_ISOLATION_DIV_CH];
...
/*		case P2:
			...
			} else if(myPs->signal[M_SIG_BMS_EOL_DELAY2] == 2
				&& myPs->signal[M_SIG_BMS_EOL_DELAY1] == 0) {
				myData->CAN.signal[can_ch1][CAN_SIG_TX_PHASE_AUX_OFF] = 28;
				myData->CAN.signal[can_ch2][CAN_SIG_TX_PHASE_AUX_OFF] = 28;
			}
