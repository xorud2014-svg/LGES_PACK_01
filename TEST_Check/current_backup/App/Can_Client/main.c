...
int main(int argc, char *argv[])
{
	...
	if(Initialize(argc, argv) < 0) return 0;
   	...
   	while(myData->AppControl.signal[myPs->misc.psSignal] == P1) {
		myPs->signal[CAN_SIG_NET_CONNECTED] = P0;
		myPs->signal[CAN_SIG_CLIENT_CONNECT] = P0;
	...
	if(Initialize(argc, argv) < 0) return 0;
   	...
   	while(myData->AppControl.signal[myPs->misc.psSignal] == P1) {
		...
		while(myData->AppControl.signal[myPs->misc.psSignal] == P1) {
			...
			//tv.tv_usec = 150000; //150mSec
			//tv.tv_usec = 1000000; //1000mSec	
...
int Send_Cmd(void)
{
	...
	if(myPs->signal[CAN_SIG_NET_CONNECTED] == P1) {
		send_cmd_ch_data();
	}
	return rtn;
}
