...
void msgParsing_App_to_COA(int fromPs,  int idx, S_MSG_VAL *RecvMsg)
{
	switch(RecvMsg->msg) {
		...
		case MSG_APP_COA_TEST_COND_CONVERSION_END:
			...
			break;
		case MSG_APP_COA_READ_TH_TABLE_SET_REPLY: //kjhw_181111
			send_cmd_th_table_set_reply(RecvMsg->val[1]);
			break;
		case MSG_APP_COA_READ_HUMIDITY_TABLE_SET_REPLY: //csk_200207
			send_cmd_humidity_table_set_reply(RecvMsg->val[1]);
			break;
		case MSG_APP_COA_READ_CAN_TH_TABLE_SET_REPLY: //ktg_220614
			send_cmd_can_th_table_set_reply(RecvMsg->val[1]);
			break;
		case MSG_APP_COA_CMD_CALI_TEMP_REPLY:			//khj_210802
			send_cmd_cali_temp_reply(RecvMsg->val[1]);
			break;
		default:
			userlog(DEBUG_LOG, psName,
...
void msgParsing_Module_to_COA(int fromPs,  int idx, S_MSG_VAL *RecvMsg)
{
	...
	switch(RecvMsg->msg) {
		...
		case MSG_MODULE_COA_DAQ_ISOLATION_REPLY:
			send_cmd_daq_isolation_reply(RecvMsg->val[1], RecvMsg->val[2]);	//ktg_210312
			break;
		//jhkw_150224s
		case MSG_MODULE_COA_DAQ_ISOLATION_REPLY2:
			send_cmd_daq_isolation_reply2(RecvMsg->val[1], RecvMsg->val[2], RecvMsg->val[3]);	//ktg_210312
			break;
		//jhkw_150224e
