...
typedef struct s_cali_tmp_cond_tag {
	...
	int					range;
	int					mode;
	unsigned int		count; //kjhw_120506 Vref x 2
...
typedef struct s_cali_tmp_data_tag {
	...
	long				set_meter_real[MAX_CALI_POINT]; //jhkw_200317
	long				check_ad[MAX_CALI_POINT];
	long				check_meter[MAX_CALI_POINT];
	long				check_meter_real[MAX_CALI_POINT]; //jhkw_200317
...
typedef struct s_calibration_tag {
	...
	int					range;
	int					mode;
	unsigned int		count; //kjhw_120504 Vref x 2
	unsigned long		shunt_value;	//jhkw_200317
