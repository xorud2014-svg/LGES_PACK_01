...
int NetworkPacket_Receive(void)
{
	...
	if(ioctl(myPs->config.fd_socket, FIONREAD, &rcv_size) < 0) {
		userlog(DEBUG_LOG, psName,
...
void make_header(char *buf, int cmd_id, int body_size)
{
	...
	sprintf((char *)&header.cmd_id, "%03d", cmd_id);
...
//	memcpy((char *)&header1, cmd, sizeof(S_TEMP_CLIENT_CMD_HEADER));
	...
	rtn = writen(myPs->config.fd_socket, packet, cmd_size + 2);
	if(rtn < 0) {
...
{
	int rtn=0, i, start_ch, Module, ch;
	float val1;
	char tmp[12];
	...
	for(i = start_ch; i < myPs->config.installedTemp; i++){
