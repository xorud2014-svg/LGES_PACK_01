...
int set_reset_mode(int can)
{
	...
	tmp2 = 0;
	tmp = inb(base + MODE);
	while((tmp != RESET_MODE) && tmp2 < 100) { //kjg_w 100ms
...
int set_normal_mode(int can, unsigned char ucModifier)
{
	...
	tmp2 = 0;
	tmp = inb(base + MODE);
	while((tmp != ucModifier) && tmp2 < 100) { //kjg_w 100ms
...
int sja1000_baud_rate(int rate, int flags)
{
...
int rt_can_set_param(int can, int baud, int extended)
{
	...
	btr0btr1 = (unsigned short)sja1000_baud_rate(baud, 0);
	...
	if(set_normal_mode(can, NORMAL_MODE) != 0) {
		rt_can_irq_on(state);
		return -3;
	}
...
int rt_can_setup(int can, int baud, unsigned char extended)
{
	unsigned char tmp, clkdivider;
	unsigned short btr0btr1;
	unsigned int base;
