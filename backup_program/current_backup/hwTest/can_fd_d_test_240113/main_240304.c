...
int exec_cmd1(int can_ch)
{
	...
	if(kjg_val[1] == 3 && kjg_val[2] == 46 && kjg_val[3] == 1
		...
		&& (kjg_val[46] == 0x00 || kjg_val[46] == 0x01)
		&& (kjg_val[47] == 0x00 || kjg_val[47] == 0x01)
		) i++;
	if(i == 2) rtn = 1;
...
int exec_cmd2(int can_ch)
{
	...
	txData.val = 80; //80ms
	outb(txData.data[0], addr+1); //set_can_filter_time
	outb(txData.data[1], addr+1);
...
int exec_cmd3(int can_ch)
{
	...
	if(kjg_val[1] == 0x03 && kjg_val[2] == 0x07
		&& kjg_val[3] == 0x03) i++;
	if(i == 2) rtn = 1;
...
int exec_t1_wr(int can_ch)
{
	...
	outb(0x10, addr+4);
	outb(can_ch, addr+4);
	outb(can_ch+1, addr+4);
	outb(can_ch+2, addr+4);
	outb(can_ch+3, addr+4);
...
int exec_t2_wr(int can_ch)
{
	...
	outb(tx_data[0], addr+4); //write with crc
	outb(tx_data[1], addr+4);
	outb(tx_data[2], addr+4);
	outb(tx_data[3], addr+4);
	outb(tx_data[4], addr+4);
	outb(tx_data[5], addr+4);
	outb(tx_data[6], addr+4);

	crc.val = CHIP_CalculateCRC16(tx_data, 7);
	outb(crc.data[1], addr+4); //h
	outb(crc.data[0], addr+4); //l
...
int exec_t2_rd(int can_ch)
{
	...
	outb(rx_data[0], addr+4); //read with crc
	outb(rx_data[1], addr+4);
	outb(rx_data[2], addr+4);
...
int main(void)
{
	...
	union short_int_tmpData adData;
    struct timeval tv;
    fd_set rfds;
	FILE *fp;
	...
	while(1) {
		...
		if(strcmp(tmp, "ver") == 0) {
			...
			if(kjg_val[0] == 0x44 || kjg_val[0] == 0x64) { //'D' or 'd'
				tmp1 = 0;
			} else tmp1 = 4;
