...
//20181219 KHK------------------------------------------------------
int Read_SOC_Tracking_File(char *psName1, int ch, int pattern_index)
{ 
...
/*
	for(i=0; i < (MAX_SOC_TRACKING_DATA - 5) - 1; i++) { //SOC 
		for(j=0; j < MAX_SOC_TRACKING_DATA; j++) { // Temp 
			if((i <  myData->testCond[ch].SOC_tracking.soc_num-1) //SOC
					&&(j <  myData->testCond[ch].SOC_tracking.temp_num)){ //Temp
				diff = (float)(myData->testCond[ch].SOC_tracking.SOC[i]
					   - myData->testCond[ch].SOC_tracking.SOC[i+1]);
				if(diff != 0){
					diff_current = (float)(myData->testCond[ch].SOC_tracking.limit_current[i][j]
						- myData->testCond[ch].SOC_tracking.limit_current[i+1][j]);
					if(diff_current != 0){
						A = diff_current/ diff;
						myData->testCond[ch].SOC_tracking.soc_tracking_data_A[i][j]
							= A;
						myData->testCond[ch].SOC_tracking.soc_tracking_data_B[i][j]
							= myData->testCond[ch].SOC_tracking.limit_current[i+1][j]
							- A * myData->testCond[ch].SOC_tracking.SOC[i+1];
					}else{
						myData->testCond[ch].SOC_tracking.soc_tracking_data_A[i][j] = 0.0;
						myData->testCond[ch].SOC_tracking.soc_tracking_data_B[i][j]
							= myData->testCond[ch].SOC_tracking.limit_current[i+1][j];
					}
				}else{
...
int read_test_cond_pattern_file_coa(char *psName1, int ch, int pattern_index)
{ //kjg_130430
#if defined __COA__
	char cmd[256], *in_delimiter = " ,\t\n\r", *token;
