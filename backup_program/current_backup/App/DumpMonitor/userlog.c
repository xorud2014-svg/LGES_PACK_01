...
void Init_Logfile(char *fname, int type)
{
	...
	memset((char *)&myData->log[type].LogPath[0], 0, 256);
	strcpy((char *)&myData->log[type].LogPath[0], pathname);
	
	memset((char *)&myData->log[type].LogFile[0], 0, 256);
	strcpy((char *)&myData->log[type].LogFile[0], filename);
...
void logopen(char *pathname, char *filename, int postfix, int type)
{
	...
	rtn = strcmp((char *)&myData->log[type].OpenLogFile[0], LogFile);
	if(rtn != 0) {//is not equal
		if(myData->log[type].OpenLogFile[0] == 0) {
			memset((char *)&myData->log[type].OpenLogFile[0], 0, 256);
			strcpy((char *)&myData->log[type].OpenLogFile[0], LogFile);
		} else {
			for(i=1; i <= 12; i++) {
...
void userlog(int type, char *name, char *fmt, ...)
{
	...
	logopen((char *)&myData->log[type].LogPath[0],
		(char *)&myData->log[type].LogFile[0], LOG_FNAME_DATE, type);
	...
	if(myData->log[type].LogFlag & LOG_2FILE) {
		if((log_fp = fopen((char *)&myData->log[type].OpenLogFile[0], "a"))
			== NULL) return;
		...
		fflush(log_fp);
		fclose(log_fp);
		chmod((char *)&myData->log[type].OpenLogFile[0], 0666);
	}
