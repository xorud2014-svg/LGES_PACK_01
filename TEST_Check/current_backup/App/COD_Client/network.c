...
{
	//char debug[MAX_P3_RECV_PACKET_LENGTH]; //kjgd
...
int CmdHeader_Check(char *rcvHeader)
{
	...
	if(header.cmd_serial > MAX_P3_CMD_SERIAL) {
		send_cmd_unknown(header.cmd_id, P3_CD_SEQ_NO_ERROR); //kjgw
...
int rcv_cmd_module_set_data(void)
{
	...
	//kjgw line, control, working mode
...
int rcv_cmd_testcond_start(void)
{
	...
	if(cmd.testCondHeader.totalStep > MAX_P3_STEP) {
		return send_cmd_response((char *)&cmd.header,
			P3_CD_TEST_STEP_COUNT_ERROR); //kjgw
	}
...
int rcv_cmd_testcond_safety(void)
{
	...
	if(myPs->signal[P3_SIG_TEST_HEADER_RCV] != P1) {
		return send_cmd_response((char *)&cmd.header,
			P3_CD_TEST_HEADER_UNRCV); //kjgw
	}
	
	memcpy((char *)&myPs->testCond.safety,
		(char *)&cmd.safety, sizeof(S_P3_TEST_COND_SAFETY)); //kjgw
...
//kjgw	myPs->testCond.step[step].faultUpper_Capacitance
...
//		= cmd.testCondPattern.faultLower_Capacitance;
	...
	myPs->testCond.pattern[pattern_index].faultLower_AmpareHour
		= cmd.testCondPattern.faultLower_AmpareHour;
//kjgw	myPs->testCond.pattern[pattern_index].faultUpper_Capacitance
//		= cmd.testCondPattern.faultUpper_Capacitance;
...
//		= cmd.testCondPattern.faultLower_Capacitance;
	...
	if(token == NULL) {
		return send_cmd_response((char *)&cmd.header, P3_CD_NACK); //kjgw
	}
...
/*kjgd	userlog(DEBUG_LOG, psName, "can_receive(%d) %ld %ld %ld %d\n",
		ch+1,
...
int send_cmd_response(char *rcvHeader, int code)
{
	...
	//cmd.response.cmdSerial = header.cmd_serial; //kjgw
...
*/
		//kjgw rtn = send_cmd_fadm_pulse_data(i);
		//kjgw rtn = send_cmd_sensor_data();
...
void send_cmd_meter_connect_reply(void)
{
	...
	cmd.state = 1; //disconnect:0, connect:1 //kjgw
